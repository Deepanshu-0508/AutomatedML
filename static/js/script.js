...
function updatePreviewTable(data) {
    ...
    data.preview.forEach(row => {
        ...
        data.columns.forEach(col => {
            const td = document.createElement("td");
            // Render an empty string if the value is null/undefined
...
uploadBtn.addEventListener("click", async () => {
    ...
    try {
        ...
        // Use helper to populate the DOM table
...
function displayIssues(issues) {
    ...
    if (issues.missing && Object.keys(issues.missing).length > 0) {
        ...
        for (const [colName, stats] of Object.entries(issues.missing)) {
            ...
            const headers = document.querySelectorAll('#previewTable th');
            headers.forEach(th => {
                if (th.textContent.trim() === colName || th.textContent.trim() === `${colName} *`) {
                    th.style.backgroundColor = '#ffe6e6'; 
                    th.style.color = '#d8000c';
                    th.textContent = `${colName} *`; 
                }
    ...
    // --- COMMIT 4: "Apply Cleaning" Button and Pipeline Loop ---
    
    // Always remove existing cleaning controls first to maintain a clean slate
    const existingControls = document.getElementById('cleaning-controls');
    if (existingControls) {
        existingControls.remove();
    }

    // Only generate cleaning controls if there are missing values remaining
    if (issues.missing && Object.keys(issues.missing).length > 0) {
        const cleaningControls = document.createElement('div');
        cleaningControls.id = 'cleaning-controls';
        ...
        // CLICK HANDLER: Gather operations payload and hit backend /clean endpoint
        applyBtn.addEventListener('click', async () => {
            const operations = {};
            const dropdowns = cleaningControls.querySelectorAll('select');
            
            // Map selected values into the exact structure expected by utils/cleaner.py
            ...
            try {
                ...
                // 1. Rebuild the visual HTML table with the fresh preview data
                updatePreviewTable(data);
                
                // 2. Recursively trigger displayIssues to handle the new dataset analysis loop
...
function displayIssues(issues) {
    ...
    headers.forEach(th => {
        ...
        if (th.dataset.colName) {
            th.textContent = th.dataset.colName; // Preserve clean original text
        }
    });
    ...
    if (missingCols.length > 0) {
        ...
        for (const [colName, stats] of Object.entries(issues.missing)) {
            ...
            // Highlight corresponding table header without corrupting column text
            headers.forEach(th => {
    ...
    // --- COMMIT 5: Cleaning Controls & Polish ---
    
    // Always remove existing cleaning controls first to prevent duplicates
    const existingControls = document.getElementById('cleaning-controls');
    if (existingControls) {
        existingControls.remove();
    }
    ...
    if (missingCols.length === 0) {
        // POLISH: Display "No missing values left" if all are cleaned
        ...
        // Generate dropdown controls for missing columns
        ...
        applyBtn.addEventListener('click', async () => {
            ...
            // POLISH: Loading Indicator State
            ...
            try {
                ...
                // POLISH: Handle case where cleaning removes all rows
                if (data.shape && data.shape[0] === 0) {
                    alert('⚠️ Warning: This cleaning operation removed all rows (0 rows remaining)! You may want to re-upload your file or choose different actions.');
                }
